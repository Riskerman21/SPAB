...
rain_columns = ['flood_aread', 'lat', 'long']

new_rain_data = pd.DataFrame([])
for column in rain_data.columns:
    for rain_column in rain_columns.columns:
        if column == rain_column:
            new_rain_data.merge(pd.DataFrame({column : rain_column['column']}))
    ...
    if rain_date in flood_dates:
        for category in rain_data.columns:
            if category != 'datetime':
                flooddata_dict[rain_date] = pd.merge([flooddata_dict[rain_date], pd.DataFrame([rain_data[category]])])